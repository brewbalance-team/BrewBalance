  ...
  return (
    <div className="bg-slate-900 p-5 rounded-3xl border border-slate-800 shadow-lg relative">
      <div className="flex justify-between items-center mb-3">
        ...
        <span
          className={`text-xs font-black px-2 py-0.5 rounded flex items-center gap-1 ${isNegative ? 'bg-red-950/50 text-red-400' : 'bg-emerald-950/50 text-emerald-400'}`}
        >
      ...
      <div className="flex justify-between items-end mb-2 px-1">
        <div className="flex flex-col">
          ...
          <span
            className={`text-2xl font-black leading-none ${isNegative ? 'text-red-400' : 'text-emerald-400'}`}
          >
            {currency}
            {Math.round(saved)}
          </span>
...
const Dashboard: React.FC<DashboardProps> = ({ statsMap, settings, streak, onUpdateSettings }) => {
  ...
  const challengeStats = useMemo(() => {
    ...
    if (isUpcoming) {
      ...
      if (isFailedMath) {
        ...
        const progressRatio =
          totalDuration > 0 ? Math.max(0, Math.min(1, elapsed / totalDuration)) : 1;
  ...
  const renderChallengeCard = () => {
    ...
    return (
      <div
        className={`mb-4 bg-gradient-to-br ${ui.bgGradient} border ${ui.borderColor} p-4 rounded-3xl relative overflow-hidden shadow-lg shrink-0 group transition-all`}
      >
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-2">
            <span
              className={`${ui.badgeBg} text-slate-950 text-[10px] font-black px-2 py-0.5 rounded uppercase tracking-wider shadow-sm flex items-center gap-1`}
            >
            ...
            <div className="flex items-center gap-2">
              ...
              {!isCardUpcoming && (
                <span
                  className={`${ui.color} text-[10px] font-bold uppercase tracking-wider opacity-80`}
                >
                  {formattedDate}
                </span>
              )}
          ...
          <div className="flex flex-col mb-2">
            <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1">
              ...
              {isCardUpcoming ? (
                <div className="flex items-center gap-2 ml-auto shrink-0 mt-1">
                  <span
                    className={`text-xs font-bold ${ui.color} bg-slate-900/40 px-2 py-1 rounded-lg border border-white/5`}
                  >
                ...
                <div className="flex items-center gap-2 ml-auto shrink-0">
                  <span className={`text-base font-bold ${ui.color} uppercase tracking-wider`}>
                    Day <span className="text-white">{challengeStats.dayNumber}</span>/
                    {challengeStats.totalDays}
                  </span>
            ...
            {isCardUpcoming && (
              <div className="mt-3 pt-3 border-t border-white/10 flex items-center justify-between gap-4 text-xs font-bold text-slate-300">
                <div className="flex flex-col">
                  <span className={`text-[9px] uppercase tracking-wider ${ui.color} opacity-70`}>
                    Starts
                  </span>
                  <span>
                    {new Date(settings.activeChallenge.startDate).toLocaleDateString(undefined, {
                      month: 'short',
                      day: 'numeric',
                    })}
                  </span>
                ...
                <div className="flex flex-col text-right">
                  ...
                  <span>
                    {new Date(settings.activeChallenge.endDate).toLocaleDateString(undefined, {
                      month: 'short',
                      day: 'numeric',
                    })}
                  </span>
      ...
      {(!settings.activeChallenge || isUpcoming) && (
        <div className="mb-2 px-1 shrink-0">
          <h2 className="text-slate-400 font-bold text-xs uppercase tracking-wider flex items-center gap-2">
            <span
              className={`w-2 h-2 rounded-full shadow-[0_0_10px_rgba(255,255,255,0.2)] ${isBudgetSet ? '' : 'bg-slate-600'}`}
              style={{ backgroundColor: isBudgetSet ? statusDetails.color : undefined }}
            ></span>
            {formattedDate}
          </h2>
        </div>
      ...
      <div className="flex-1 flex flex-col min-h-0 overflow-y-auto custom-scrollbar">
        {isChallengeDay && challengeStats ? (
          ...
          <div className="flex flex-col gap-3 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <SavingsProgressBar
              label="Daily Savings Progress"
              saved={todayStats.remaining}
              total={todayStats.baseBudget}
              currency={settings.currency}
            />
            <SavingsProgressBar
              label="Total Challenge Progress"
              saved={challengeStats.totalSavedSoFar}
              total={challengeStats.totalBudget}
              currency={settings.currency}
            />
          </div>
        ) : (
          ...
          <div
            className={`relative p-5 rounded-[2.5rem] border ${statusDetails.border} ${statusDetails.bg} transition-all duration-500 shadow-lg flex flex-col grow shrink-0 mb-4`}
          >
            {/* Card Header: Label + Streak */}
            <div className="flex justify-between items-start mb-4 shrink-0">
              <h2
                className={`text-base font-black ${statusDetails.text} uppercase tracking-widest mt-2`}
              >
                {statusDetails.label}
              </h2>
              ...
              <div className="flex-1 flex flex-col justify-start gap-4 overflow-hidden">
                ...
                <div className="bg-slate-900/40 p-4 rounded-2xl border border-white/5 backdrop-blur-sm shrink-0">
                  ...
                  <div className="flex items-center justify-between px-1">
                    ...
                    <div className="flex flex-col items-center gap-1">
                      <span
                        className={`font-black text-xl ${todayStats.rollover < 0 ? 'text-red-400' : 'text-emerald-400'}`}
                      >
                        {todayStats.rollover < 0 ? '-' : ''}
                        {currency}
                        {Math.abs(Math.round(todayStats.rollover))}
                      </span>
                ...
                <div className="text-center shrink-0 flex-1 flex flex-col justify-center">
                  ...
                  <p
                    className={`text-6xl sm:text-7xl font-black tracking-tighter drop-shadow-sm leading-none ${statusDetails.text}`}
                  >
                    {currency}
                    {Math.round(todayStats.remaining)}
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="flex items-center justify-between mt-auto pt-2 border-t border-white/5 shrink-0">
              ...
              <div className="h-32 w-32 relative">
                ...
                <div className="absolute inset-0 flex items-center justify-center text-xs font-black text-slate-400 uppercase">
                  {isBudgetSet
                    ? `${Math.round((todayStats.spent / (todayStats.totalAvailable || 1)) * 100)}%`
                    : '--'}
                </div>
              </div>
            </div>
          ...
          {settings.activeChallenge && !isUpcoming ? (
            <div className="mx-2 mb-2 py-4 px-4 bg-gradient-to-r from-emerald-500/10 to-emerald-600/10 border border-emerald-500/20 rounded-2xl flex items-center justify-center gap-3 shadow-sm text-center">
              ...
              <p className="text-emerald-100 font-bold text-sm">
                Push your limits! Save extra today for your{' '}
                <span className="text-white">{settings.activeChallenge.purpose || 'goal'}</span>.
              </p>
            </div>
          ) : (
                ...
                {!showEndConfirm ? (
                  ...
                  <div className="bg-red-950/20 border border-red-900/50 p-4 rounded-2xl space-y-3 animate-in fade-in slide-in-from-bottom-2">
                    <p className="text-center text-red-400 text-xs font-bold">
                      Are you sure you want to end this challenge early?
                    </p>
