...
const App: React.FC = () => {
  ...
  const statsMap = useMemo(() => {
    ...
    const localEnd = new Date(futureDate.getTime() - offset * 60 * 1000)
      .toISOString()
      .split('T')[0];
  ...
  return (
    <div className="min-h-screen bg-slate-950 font-sans text-slate-100 flex justify-center">
      <div className="w-full max-w-md h-[100dvh] bg-slate-950 shadow-2xl relative flex flex-col overflow-hidden sm:border-x sm:border-slate-800">
        ...
        <header
          className="px-6 pt-[max(2.5rem,env(safe-area-inset-top))] pb-4 bg-slate-950/80 backdrop-blur-md sticky top-0 z-30 flex justify-between items-center border-b border-slate-800 transition-all"
          {...testId('app-header')}
        >
          <div className="flex items-center gap-3">
            <div
              className="w-14 h-14 rounded-xl overflow-hidden shadow-lg border border-slate-800 shrink-0"
              {...testId('app-logo')}
            >
              {settings.logo ? (
