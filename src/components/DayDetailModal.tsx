...
const DayDetailModal: React.FC<DayDetailModalProps> = ({
  ...
  // Re-initialize local state when stats change (intentional prop-to-state sync)
  /* eslint-disable react-hooks/set-state-in-effect */
  useEffect(() => {
    setBudget((stats?.baseBudget ?? 0).toString());
    setRollover((stats?.rollover ?? 0).toString());
  }, [stats?.date, stats?.baseBudget, stats?.rollover]);
      ...
      >
        ...
        <div className="space-y-4 mb-6">
          ...
          <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 flex justify-between items-center">
            ...
            <div
              className={`text-2xl font-black ${stats.remaining >= 0 ? 'text-emerald-400' : 'text-red-400'}`}
            >
              {currency}
        ...
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div>
            <label className="flex items-center justify-between text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
              <span className="flex items-center gap-1">
                <TrendingUp size={12} /> From Previous Day{' '}
                {stats.isCustomRollover && <span className="text-amber-500">(Custom)</span>}
              </span>
            ...
            <p className="text-[9px] text-slate-600 mt-1 pl-1">
              Adjusting this overrides the carry-over from yesterday.
            </p>
          </div>
          ...
          <div>
            <label className="flex items-center justify-between text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
              <span>
                Base Budget{' '}
                {stats.isCustomBudget && <span className="text-amber-500">(Custom)</span>}
              </span>
