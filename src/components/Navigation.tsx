      ...
      <div className="bg-slate-900/90 backdrop-blur-xl border border-slate-800/80 rounded-3xl px-2 py-2 shadow-2xl shadow-black/50 flex items-center justify-between w-full max-w-sm ring-1 ring-white/5">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <div
                className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-10 h-10 bg-slate-800 rounded-xl scale-0 transition-transform duration-300 ${isActive ? 'scale-100' : 'group-hover:scale-50'}`}
              ></div>
              <div
                className={`relative flex flex-col items-center z-10 ${isActive ? 'text-amber-400' : 'text-slate-500'}`}
              >
                <Icon
                  size={22}
                  strokeWidth={isActive ? 2.5 : 2}
                  className="transition-transform duration-300 mb-1"
                />
