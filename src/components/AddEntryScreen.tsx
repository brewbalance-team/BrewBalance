...
const AddEntryScreen: React.FC<AddEntryScreenProps> = ({
  ...
  const handleStartChallenge = (data: Omit<Challenge, 'id' | 'status'>) => {
    ...
    if (settings.activeChallenge) {
      if (
        !confirm(
          'You already have an active challenge. Starting a new one will cancel the current one. Continue?',
        )
      ) {
        return;
      }
        ...
        {mode === 'expense' ? (
              ...
              <div className="space-y-4">
                ...
                <div className="w-full overflow-hidden">
                  ...
                  <input
                    ...
                    className="w-full max-w-full min-w-0 p-3 bg-slate-950 rounded-xl border border-slate-800 focus:border-amber-500 outline-none font-bold text-slate-200 scheme-dark text-sm"
          ...
          <div className="flex flex-col h-full gap-4">
            <div className="bg-slate-900 p-5 rounded-[2rem] shadow-lg border border-slate-800 flex-1">
              ...
              <ChallengeForm onSubmit={handleStartChallenge} submitLabel="Start Challenge" />
            </div>
          </div>
        )}
      </div>
