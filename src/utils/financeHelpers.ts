...
export const calculateStats = (
  ...
  const lastEntryDate =
    sortedEntries.length > 0 ? sortedEntries[sortedEntries.length - 1]!.date : settings.startDate;
  ...
  while (currentDateStr <= endDateCalcStr) {
    ...
    statsMap[currentDateStr] = {
      ...
      isChallengeDay,
      ...(isChallengeDay
        ? { challengeName: challenge?.name, challengeSavedSoFar: challengeAccumulatedSavings }
        : {}),
    };
 ...
 */
export const calculateChallengeBudgetSoFar = (
  challenge: Challenge,
  settings: Settings,
